...
enum Url {
    ...
    private static func makeURL(_ path: String) -> NSURL {
        let baseURL = Application.BaseURL
        let cleanBaseURL = baseURL.hasSuffix("/") ? String(baseURL.dropLast()) : baseURL
        let cleanPath = path.hasPrefix("/") ? String(path.dropFirst()) : path
        
        return NSURL(string: "\(cleanBaseURL)/\(cleanPath)")!
    }

    // MARK: - API

    static var contactTags: NSURL {
        return makeURL("customers/tags")
    }

    static var createTags: NSURL {
        return makeURL("customers/tags/store")
    }

    // MARK: - CREATE CONTACT

    static var createContact: NSURL {
        return makeURL("customers/store")
    }
}
