    ...
    func setNavigation(){
        ...
        setNavigationBarForButtons(controller: self, title: str.strOderTitle, isTransperent: true, hideShadowImage: true, leftIcon: "icon_back", rightIcon: ["icon_Filter", "icon_notification"], isFilter: self.checkFilter()) {
            ...
            if SelectTag == 1{
                ...
                let alert = UIAlertController(title: "Clear All Notifications", message: "Are you sure you want to clear all notification alerts?", preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: str.yes, style: .default,handler: { (Action) in
                    GlobalMainConstants.appDelegate?.updateNotificationApi(NotificationParameater: NotificationParameater(order_id: ""))
                }))
                ...
                view.selectNotificationType = self.selectNotificationType
...
extension OrderListViewController : FilterProtocol{
    func SelectFilter(categoryID: Int, strStatus: String, strPaymentType: String, strDeliveryType: String, strNotificationType: String) {
        ...
        if self.selectNotificationType != "All"{
            let arr = self.splitNotificationsByOrderMatch(orders: self.arrMainOrderList, notifications: arrNotifications)
            
            if self.selectNotificationType != "Unread"{
                self.arrOrderList = arr.notMatched
            }
            else{
                self.arrOrderList = arr.matched
            }
            
        }
        else{
            self.arrOrderList = self.arrMainOrderList
        }
...
extension OrderListViewController{
    ...
    func splitNotificationsByOrderMatch(
        orders: [OrdersListModel],
        notifications: [NotificationsModel]
    ) -> (matched: [OrdersListModel], notMatched: [OrdersListModel]) {

        // Collect all valid order IDs
        let notificationIds = Set(notifications.compactMap { $0.order_id })

        var matched: [OrdersListModel] = []
        var notMatched: [OrdersListModel] = []
        ...
        for order in orders {
            ...
            if notificationIds.contains(orderId) {
...
extension OrderListViewController : UITableViewDelegate, UITableViewDataSource, OrderDetailsDelegate, LicenseUploadDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "OrderListCell") as? OrderListCell{
            ...
            cell.viewNotificaiton.backgroundColor = .redText
            cell.viewNotificaiton.viewCorneRadius(radius: 0.0, isRound: true)
            cell.viewNotificaiton.isHidden = true
            ...
            let MenuID = arrNotifications.compactMap { $0.order_id }
            if MenuID.firstIndex(of: objData.id ?? 0) != nil{
                cell.viewNotificaiton.isHidden = false
            }
            ...
            for objProduct in objData.arrProduct{
                ...
                if let variant = objProduct.objProductData?.product_variant{
                    productVariant = variant
                }
                
                
                if strProduct != "" {
                    strProduct = "\(strProduct)\n• \(objProduct.product_name ?? "") - \(productVariant.capitalized)"
                }
                else{
                    strProduct = "• \(objProduct.product_name ?? "") - \(productVariant.capitalized)"
                }
            }
...
//            cell.btnPickupStatus.addTarget(self, action: #selector(self.btnPickupStatusClicked(_:)), for: .touchUpInside)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let newViewController = storyBoard.instantiateViewController(withIdentifier: "OrderDetailsViewController") as? OrderDetailsViewController{
            ...
            newViewController.OrderID = "\(objData.id ?? 0)"
...
//            SDKUserDefault.saveMappableArray(self.arrOrderList, for: kFileStorageName.kOrderList.rawValue)
//                
//            //RELOAD CELL
//            self.tblView.reloadRows(at: [IndexPath(row: selectIndex, column: 0)], with: .none)
//          }
//    }
    ...
    @objc func UpdateCheckListsProduct(notification: NSNotification) {
        guard !arrOrderList.isEmpty else { return }
        
        guard
            let userInfo = notification.userInfo,
            let arrUpdateCheckList = userInfo["checklist_data"] as? [NoteModel],
            let selectIndex = userInfo["index"] as? Int,
            let strType = userInfo["type"] as? Bool,
            arrOrderList.indices.contains(selectIndex)
        else {
            return
        }
        
        var objData = arrOrderList[selectIndex]
        
        for obj in arrUpdateCheckList {
            let menuIDs = objData.arrProduct.map { $0.id }
            
            if let index = menuIDs.firstIndex(of: obj.productID),
               objData.arrProduct.indices.contains(index) {
                ...
                objData.arrProduct[index] = objProduct
            }
        }
        
        arrOrderList[selectIndex] = objData
        
        SDKUserDefault.saveMappableArray(arrOrderList, for: kFileStorageName.kOrderList.rawValue)
        
        DispatchQueue.main.async {
            let indexPath = IndexPath(row: selectIndex, section: 0)
            
            if self.tblView.numberOfSections > 0 &&
                self.tblView.numberOfRows(inSection: 0) > selectIndex {
