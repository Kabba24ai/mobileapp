...
extension UIView {
    func roundCornersView(onTopLeft tl: Bool, topRight tr: Bool, bottomLeft bl: Bool, bottomRight br: Bool, radius: Float) {
        if tl || tr || bl || br {
            ...
            maskPath = UIBezierPath(roundedRect: self.bounds , byRoundingCorners: corner, cornerRadii: CGSize(width: CGFloat(radius), height: CGFloat(radius)))
...
class ActualGradientButton: UIButton {
    ...
    private lazy var gradientLayer: CAGradientLayer = {
        ...
        l.frame = self.bounds
        l.colors = [UIColor.clear.cgColor, UIColor.primaryView?.cgColor ?? UIColor.clear.cgColor]
...
extension UIView{
    func setTheTextView(bgColor : UIColor = .clear, size : CGFloat = 1){
        self.backgroundColor = .clear
        self.viewCorneRadius(radius: 5, isRound: false)
        self.viewBorderCorneRadius(borderColour: bgColor, size : size)
    }
    ...
    func viewBorderCorneRadius(borderColour : UIColor?, size : CGFloat = 1){
    ...
    func viewBorderCorneRadius(radius : CGFloat, borderColour : UIColor?){
        self.backgroundColor = .clear
        self.layer.cornerRadius = radius
