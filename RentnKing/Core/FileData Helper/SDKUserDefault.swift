...
extension SDKUserDefault {
    ...
    static func saveNSObjectArray(_ notes: [NoteModel], key: String, imageQuality: CGFloat = 0.8) {
        let arr = notes.map { $0.toDict(imageQuality: imageQuality) }

        guard JSONSerialization.isValidJSONObject(arr),
              let data = try? JSONSerialization.data(withJSONObject: arr, options: []) else {
            print("❌ Failed to convert notes to JSON Data.")
            return
        }
        ...
        print("✅ Saved notes in MMKV. count=\(notes.count), key=\(key)")
    }
    ...
    static func getNSObjectArray(key: String) -> [NoteModel] {
        // guard let data = mmkv?.data(forKey: key) else { return [] }
        guard let data = SDKUserDefault.shared.mmkv?.data(forKey: key) else { return [] }

        guard let obj = try? JSONSerialization.jsonObject(with: data, options: []),
              let arr = obj as? [[String: Any]] else {
            print("❌ Failed to decode notes JSON.")
            return []
        }

        return arr.map { NoteModel.fromDict($0) }
    }
    
    static func getMappableArray<T: NSObject>(_ type: T.Type, for key: String) -> [T]? {
        ...
        do {
            ...
            print("❌ Failed to load NSObject array: \(error.localizedDescription)")
        }
    ...
    // MARK: - Save WebView Data
    static func saveWebviewData(_ htmlString: String, for key: String, order_id: String) {
        SDKUserDefault.shared.mmkv?.set(htmlString, forKey: "\(key)_\(order_id)")
    }
    
    // MARK: - Get WebView Data
    static func getWebviewData(_ key: String, order_id: String) -> String? {
        var cacheData = ""
        if let cachedHTML = SDKUserDefault.shared.mmkv?.string(forKey: "\(key)_\(order_id)") {
            cacheData = cachedHTML
        }
        return cacheData
    }
